            ...
            else
            {
                ...
                if ((hunter.skill as HunterSkill).TrapCount > 0)
                {
                    if (Input.GetKeyDown(interactionKey)&&!castingSystem.IsCoroutineRunning)
                    {
                        ...
                        hunter.photonView.RPC("ChangeSkillBehaviorTo_RPC", RpcTarget.AllViaServer);
                        castingSystem.StartCasting(CastingSystem.Cast.CreateByTime(3.0f,coolTime : 5.0f), new CastingSystem.CastFuncSet(null, RunningCondition, PauseAction, FinishAction) ); // RunningCondition : Input.getKey / PauseAction : Idle로 바꿔줌 /  FinishAction : Idle 바꿔주고 설치
                    }
                }
            }
